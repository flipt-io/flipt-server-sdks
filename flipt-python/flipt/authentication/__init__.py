...
class KubernetesAuthentication(AuthenticationStrategy):
    ...
    def authenticate(self, headers: dict[str, str]) -> None:
        ...
        try:
            response = requests.post('http://flipt:8080/auth/v1/method/kubernetes/serviceaccount', json=payload, timeout=5)
        except requests.exceptions.RequestException as e:
            raise RuntimeError(f'Failed to authenticate with Flipt.') from e
        ...
        try:
            ...
            raise RuntimeError(f'Failed parsing authentication response.') from e
